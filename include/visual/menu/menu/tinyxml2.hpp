...
private:
    ...
    void EnsureCapacity(int cap)
    {
        ...
        if (cap > _allocated)
        {
            ...
            memcpy(newMem, _mem, sizeof(T) * _size); // warning: not using
                                                     // constructors, only works
                                                     // for PODs
            if (_mem != _pool)
            {
                delete[] _mem;
...
public:
    ...
    */
    ...
    /** Accept a hierarchical visit of the nodes in the TinyXML-2 DOM. Every
       node in the
...
public:
    ...
    /// Gets the line number the attribute is in, if the document was parsed
    /// from a file.
    int GetLineNum() const
    {
        return _parseLineNum;
    ...
    */
    ...
    /** QueryIntValue interprets the attribute as an integer, and returns the
       value
        in the provided parameter. The function will return XML_SUCCESS on
       success,
...
template <class NodeType, int PoolElementSize> inline NodeType *XMLDocument::CreateUnlinkedNode(MemPoolT<PoolElementSize> &pool)
{
