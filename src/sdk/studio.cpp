...
void CStudioHdr::CActivityToSequenceMapping::Initialize(CStudioHdr *__restrict pstudiohdr)
{
    ...
    for (UtlHashHandle_t handle = m_ActToSeqHash.GetFirstHandle(); m_ActToSeqHash.IsValidHandle(handle); handle = m_ActToSeqHash.GetNextHandle(handle))
    {
...
bool CStudioHdr::CActivityToSequenceMapping::ValidateAgainst(const CStudioHdr *RESTRICT pstudiohdr) RESTRICT
{
    if (m_bIsInitialized)
    {
        return m_expectedPStudioHdr == pstudiohdr->GetRenderHdr() && m_expectedVModel == pstudiohdr->GetVirtualModel();
    }
    else
    {
