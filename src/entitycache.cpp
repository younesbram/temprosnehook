    ...
    if (previous_max == max && previous_ent == current_ents)
    {
        for (auto &[key, val] : array)
        {
            ...
            if (CE_GOOD((&val)))
            {
                ...
                if ((bool) hacks::warp::dodge_projectile && CE_GOOD(g_pLocalPlayer->entity) && val.m_Type() == ENTITY_PROJECTILE && val.m_bEnemy() && std::find_if(proj_map.begin(), proj_map.end(), [=](const auto &item) { return std::get<1>(item) == &val; }) == proj_map.end())
                    dodgeProj(&val);
            }
        }
    }
    else
    {
        for (u_int16_t i = 0; i <= max; ++i)
        {
            ...
            if (CE_GOOD((&array[i])))
            {
                ...
                if ((bool) hacks::warp::dodge_projectile && CE_GOOD(g_pLocalPlayer->entity) && array[i].m_Type() == ENTITY_PROJECTILE && array[i].m_bEnemy() && std::find_if(proj_map.begin(), proj_map.end(), [=](const auto &item) { return std::get<1>(item) == &array[i]; }) == proj_map.end())
                    dodgeProj(&array[i]);
            }
        }
    }
