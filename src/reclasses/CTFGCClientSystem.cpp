...
CTFGCClientSystem *CTFGCClientSystem::GTFGCClientSystem()
{
    ...
    static uintptr_t addr1                          = CSignature::GetClientSignature("55 B8 ? ? ? ? 89 E5 5D C3 8D B6 00 00 00 00 55 A1 ? ? ? ? 89 E5 5D C3 "
...
void CTFGCClientSystem::AbandonCurrentMatch()
{
    ...
    static uintptr_t addr1                              = CSignature::GetClientSignature("55 89 E5 57 56 8D 75 ? 53 81 EC ? ? ? ? C7 04 24");
...
bool CTFGCClientSystem::BConnectedToMatchServer(bool flag)
{
    ...
    static uintptr_t addr                                       = CSignature::GetClientSignature("55 89 E5 53 80 7D ? ? 8B 55 ? 75");
...
bool CTFGCClientSystem::BHaveLiveMatch()
{
    ...
    static uintptr_t addr = CSignature::GetClientSignature("55 31 C0 89 E5 53 8B 4D ? 0F B6 91");
    if (!addr)
    {
        logging::Info("CTFGCClientSystem::BHaveLiveMatch() calling NULL!");
        addr = CSignature::GetClientSignature("55 31 C0 89 E5 53 8B 4D ? 0F B6 91");
        return true;
    }
...
int CTFGCClientSystem::JoinMMMatch()
{
    ...
    static uintptr_t addr = CSignature::GetClientSignature("55 89 E5 56 53 83 EC ? 8B 5D ? 0F B6 83 ? ? ? ? 89 C2");
    if (!addr)
    {
        logging::Info("CTFGCClientSystem::JoinMMMatch() calling NULL!");
        addr = CSignature::GetClientSignature("55 89 E5 56 53 83 EC ? 8B 5D ? 0F B6 83 ? ? ? ? 89 C2");
        return true;
    }
