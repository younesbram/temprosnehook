...
namespace setupbones_reconst
{
    ...
    bool SetupBones(IClientEntity *ent, matrix3x4_t *pBoneToWorld, int boneMask)
    {
        ...
        int *entity_flags = reinterpret_cast<int *>((uintptr_t)ent + 400);
        *entity_flags |= 1 << 3; // EFL_SETTING_UP_BONES
        ...
        if (*m_pIk)
        {
            ...
            (*m_pIk)->Init(pStudioHdr, angles, adjOrigin, g_GlobalVars->curtime, 0, boneMask);
            ...
            static CalculateIKLocks_t CalculateIKLocks = reinterpret_cast<CalculateIKLocks_t>(CalculateIKLocks_sig);
        ...
        Studio_BuildMatrices(pStudioHdr, angles2, adjOrigin, pos, q, -1, NET_FLOAT(ent, netvar.m_flModelScale), pBoneToWorld, boneMask);
