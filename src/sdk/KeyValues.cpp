...
public:
    ...
    // Allows you to keep the same stack level, but change the name as you parse
    // peers
...
// Uncomment this line to hit the ~CLeakTrack assert to see what's looking like
// it's leaking #define LEAKTRACK
...
bool KeyValues::LoadFromFile(IBaseFileSystem *filesystem, const char *resourceName, const char *pathID, bool refreshCache)
{
...
#else
    /*
    People are cheating with the keyvalue cache enabled by doing the below, so
    disable it.
    ...
    */
    ...
    // If pathID is null, we cannot cache the result because that has a weird
    // iterate-through-a-bunch-of-locations behavior.
    ...
    // The cache relies on the KeyValuesSystem string table, which will only be
    // valid if we're using classic mode.
...
KeyValues *KeyValues::FindKey(const char *keyName, bool bCreate)
{
    ...
    {
        lastItem = dat; // record the last item looked at (for if we need to
...
//			Name is guaranteed to be an integer, of value 1 higher than the
...
const char *KeyValues::GetString(const char *keyName, const char *defaultValue)
{
    ...
    if (dat)
    {
        ...
        case TYPE_WSTRING:
        {
            // convert the string to char *, set it for future use, and return
            // it
            char wideBuf[512];
...
void KeyValues::SetStringValue(char const *strValue)
{
    ...
    delete[] m_sValue;
    // make sure we're not storing the WSTRING  - as we're converting over to
    // STRING
    delete[] m_wsValue;
    m_wsValue = NULL;
...
void KeyValues::SetString(const char *keyName, const char *value)
{
    ...
    if (dat)
    {
        ...
        // delete the old value
        delete[] dat->m_sValue;
        // make sure we're not storing the WSTRING  - as we're converting over
        // to STRING
...
void KeyValues::SetWString(const char *keyName, const wchar_t *value)
{
    ...
    if (dat)
    {
        // delete the old value
        delete[] dat->m_wsValue;
        // make sure we're not storing the STRING  - as we're converting over to
        // WSTRING
        delete[] dat->m_sValue;
        dat->m_sValue = NULL;
...
{
    // This code used to be recursive, which was more elegant. Unfortunately, it
    // also blew the stack for large KeyValues. So now we have the iterative
    // version which is uglier but doesn't blow the stack. This uses
    ...
    while (nodeQ.Count() > 0)
    {
        ...
        // Process all the siblings of the current node. If anyone has a child,
        // add it to the queue.
        while (cs.src)
        {
...
//-----------------------------------------------------------------------------
// Purpose: Copies a single KeyValue from src to this, using the provided
// temporary buffer if the keytype requires it. Does NOT recurse.
//-----------------------------------------------------------------------------
...
void KeyValues::RecursiveLoadFromBuffer(char const *resourceName, CUtlBuffer &buf)
{
    ...
    // We maintain the pointer to the last child here, so we don't have to
    // re-locate it each time we append the next subkey, which causes O(N^2)
    // time
    KeyValues *pLastChild = FindLastSubKey();
    ;

    // Keep parsing until we hit the closing brace which terminates this block,
    // or a parse error
    while (1)
    {
        bool bAccepted = true;
...
#ifdef POSIX
            // strtod supports hex representation in strings under posix but we
            // DON'T want that support in keyvalues, so undo it here if needed
